...
public final class RefreshCoordinator {
	...
	public static void refresh(Minecraft client) {
		...
		if (chatTranslationReady || changed.contains(TextType.CHAT)) {
			if (client.gui != null && client.gui.getChat() instanceof ChatComponentAccessor accessor) {
		...
		if (client.screen instanceof AdvancementsScreen screen) {
			long now = System.currentTimeMillis();
		...
		if (client.screen instanceof net.minecraft.client.gui.screens.dialog.DialogScreen<?> dialog) {
			long now = System.currentTimeMillis();
